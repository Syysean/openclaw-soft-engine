...
对比原生 OpenClaw 与 Soft Engine 执行层。

| 能力 | 原生 OpenClaw | Soft Engine v2.0 |
| :--- | :---: | :---: |
| **执行模型** | 线性 Prompt 流程 | 基于 DAG 的执行流程（`[PLAN]` / `[CHECKPOINT]`） |
| **模型路由** | 单模型或手动切换 | 自动多专家路由（文本 / 视觉 / 推理 / 代码） |
| **限流处理** | 共享 API Key，易受 429 影响 | 按路由隔离 API Key |
| **内存安全** | 缺少内置保护 | 512MB 容器内存限制 + 45 秒工具超时 |
| **多模态安全** | 缺少载荷校验 | 跨模态清洗，避免无效请求 |
| **生产可用性** | 需要自行加固 | 预置防护与部署模式 |
